...
def load_room(name): #поступает имя комнаты
    """
    There is a potential security problem here.
    Who gets to set name? Can that expose a variable?
    """
    return globals().get(name) #возвращает объект класса Room

def name_room(room): # поступает объект класса Room
    """
    Same possible security problem. Can you trust room?
    What's a better solution than this globals lookup?
    """
    for key, value in globals().items():
        # value - объект класса; key - переменная, за которой закреплён объект
        if value == room:
            return key # возвращает имя переменной
