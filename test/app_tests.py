...
def test_endtry():
    """при переходе на страницу концовки, должно случаться обновление
    числа минимальных попыток в записи игрока
    """
    # список комнат, при переходе в которые должна происходить
    # перезапись числа попыток
    romms = ['cor_dodge_death',
             'cor_shoot_death',
             'armory_death',
             'bridge_death',
             'the_end_loser',
             'the_end_winner',]
    # подключим контекст запроса для работы с сессией
